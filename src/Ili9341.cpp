		...
		uint8_t led)
{
    ...
    spi_set_baudrate(pSpi, spi_clock_freq);
	//spi_set_format(pSpi, 16, SPI_CPOL_0, SPI_CPHA_0, SPI_MSB_FIRST);
...
void Ili9341::setScreenColour (
	uint16_t colour16bit
) {
	for (int length = 0; length < PIX_HEIGHT * PIX_WIDTH; ++length) {
		imageBuffer[length] = colour16bit;
	}
}
...
void Ili9341::drawBitmap16(
	...
	for (int idx_y = y; (idx_y < y + hei) && (idx_y < PIX_HEIGHT); ++idx_y) {
		for (int idx_x= x; (idx_x < x + wid) && (idx_x < PIX_WIDTH); ++idx_x) {
			PIX(idx_x, idx_y) = bitmap[idx_x + ((idx_y - y) * wid)];
		}
	}
}
...
void Ili9341::drawBitmap32(
	...
	for (int idx_y = y; idx_y < hei + y; idx_y++) { 
		for (int idx_x = x; idx_x < wid + x; idx_x++) {
			...
			PIX(idx_x, idx_y) = RGBAto16bit(red, grn, blu, alp);
		}
	 }
}
...
void Ili9341::drawBitmapRGB8(
	...
	for (int idx_y = y; idx_y < hei + y; idx_y++) { 
		for (int idx_x = x; idx_x < wid + x; idx_x++) {
			...
			PIX(idx_x, idx_y) = RGBto16bit(red, grn, blu);
		}
	 }
