...
#define MHz                 1000000L
extern volatile uint16_t* imageBuffer;

extern uint32_t g_bmp_width;
extern uint32_t g_bmp_height;
...
#define PIX(x, y) (imageBuffer[((size_t)(y) * (size_t)PIX_WIDTH) + (size_t)(x)])

// uint16_t* PIX(int x, int y) {
// 	// Row-major indexing helpers
// 	return (uint16_t*) imageBuffer[(y)*PIX_WIDTH + (x)];
// }
...
public:
	...
	/***
	* Print the image buffer on the screen
	*/
	void writeImageBuffer (
	);

	/***
	 * Set the Screen to a specific Colour
	 * @param colour16bit Colour to set on screen
	 */
